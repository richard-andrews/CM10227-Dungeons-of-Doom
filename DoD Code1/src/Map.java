...
/**
 * A class that allows the user to select the current map that they want to play as well as, reads and contains in
 * memory the map of the game
 */
public class Map {
	 ...
	 */
	protected String getMapChoice() {
		/* Initialise the scanner object to read from the text file*/
		...
		* Filter all files so that only text files are added to the map files array list. */
		...
		for (int mapCounter = 0; mapCounter < (dungeonFileNames.size()); mapCounter++) {
			try {
				...
				System.out.println(mapCounter + ". " + currentMapName);
		...
		int mapNo = 0;
		System.out.println("Please enter the number of the map you would like to play");
		...
		if (mapNo > (dungeonNames.size() -1)) {
			System.out.println("Your response has been adjusted to the closest valid number");
		...
		System.out.println("You are playing " + dungeonNames.get(mapNo) + "\n");
